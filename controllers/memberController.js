...
/** Default join date for new members (override with DEFAULT_MEMBER_JOIN_DATE in .env). */
const DEFAULT_MEMBER_JOIN_DATE = process.env.DEFAULT_MEMBER_JOIN_DATE || '2026-01-01';

function parseJoinDateOrDefault(value) {
  if (value) return new Date(value);
  return new Date(`${DEFAULT_MEMBER_JOIN_DATE}T12:00:00.000Z`);
}
...
export const createMember = async (req, res) => {
  try {
    ...
    const member = await Member.create({
      ...
      joinDate: parseJoinDateOrDefault(joinDate),
...
export const updateMember = async (req, res) => {
  try {
    ...
    if (phone !== undefined) member.phone = phone ? String(phone).trim() : '';
    if (email !== undefined) {
...
export const createBulkMembers = async (req, res) => {
  try {
    ...
    const joinDate = parseJoinDateOrDefault(null);
    const created = [];
    const errors = [];
    ...
    for (let i = 0; i < rawMembers.length; i++) {
      ...
      const phone = (row.phone || row.contact || '').trim();
